        ...
        <div className={styles.main_wrap}>
            <main>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <span className={styles.title}>필수입력</span>
                    <br />
                        ...
                        <div className={styles.bigbox}>
                            ...
                            <div id={styles.birth_box} className={styles.box}>
                                <input
                                    ...
                                    min="1925-01-01"
                                    max="2006-12-31"
                                    placeholder="생년월일을 8자로 적어주세요"
                                    required
                                />
                            </div>
                        </div>
                        {addresses.map((item) => (
                            <div key={item.id} className={`${styles.bigbox} ${styles.address_bigbox}`}>
                                <div
                                    className={styles.box}
                                    onClick={() => openPostcode(item.id)} // 주소 검색, item.id 기반
                                >
                                    <p
                                        className={`${styles.input} ${styles.addressEnglishText}`}
                                        style={{
                                            color: item.address ? "#000" : "#aaa",
                                        }}
                                    >
                                        {item.address ? item.address : "영문주소"}
                                    </p>
                                    <input
                                        type="text"
                                        value={item.address}
                                ...
                                <div className={styles.box} onClick={() => openPostcode(item.id)}>
                                    <p
                                        className={`${styles.input} ${styles.postcodeText}`}
                                        style={{
                                            color: item.zip_code ? "#000" : "#aaa",
                                        }}
                                    >
                                        {item.zip_code ? item.zip_code : "우편번호"}
                                    </p>
                                    <input
                                        type="text"
                                        value={item.zip_code}
                    ...
                    <p className={styles.title} id={styles.title2}>
                        선택입력
                    </p>
