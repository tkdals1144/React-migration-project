...
function Header({ auth, refreshAuth }) {
    const { logout } = useLogout(refreshAuth);
    const [alarm, setAlarm] = useState(false);
    const [openDropdown, setOpenDropdown] = useState(false);
    return (
        <>
            <header>
                {/* 일반적인 css 는 전역범위를 가진다 */}
                {/* id를 객체방식으로 전달 */}
                <div id={styles.logo_wrap}>
                    {/* 객체도 가능하지만 기본적으로 90%는 문자열 방식으로 전송 */}
                    <Link to="/main">
                ...
                <div id={styles.wrap_wrap}>
                    <div id={styles.wrap1}>
                        ...
                        <div id={styles.header_icon_wrap}>
                            ...
                            <Link to="/wishlist">
                                <img
                                    src="/img/header_wishlist.svg"
                                    alt="Wishlist"
                                    id={styles.header_wishlist}
                            ...
                            <img
                                src="/img/header_alarm.svg"
                                alt=""
                                id={styles.header_alarm}
                                className={styles.header_icon}
                                onClick={() => setAlarm(!alarm)}
                            />
                            {alarm && <ul id={styles.header_alarm_dropdown}></ul>}

                            {/* <!-- 프로필 아이콘 --> */}
                            <img
                                src="/img/header_profile.svg"
                                alt=""
                                id={styles.header_profile}
                                className={styles.header_icon}
                        ...
                        <!-- 장바구니 아이콘 --> */}
                            <Link to="/cart/view">
                                <img
                                    src="/img/header_bucket.svg"
                                    alt=""
                                    id={styles.header_bucket}
                                    className={styles.header_icon}
                                />
                                <span
                                    id={styles.cart_quantity}
                    ...
                    <div id={styles.wrap2}>
                        <ul id={styles.header_nav}>
                            <li id={styles.header_all} className={styles.header_list}>
                                ALL
                            </li>
                            <li id={styles.header_brand} className={styles.header_list}>
                                BRAND
                            </li>
                            <li id={styles.header_fta} className={styles.header_list}>
                                FTA
                            </li>
                            <li id={styles.header_tax} className={styles.header_list}>
                                LOW PRICE
                            </li>
                            <li id={styles.header_community} className={styles.header_list}>
                                COMMUNITY
                            </li>
                        </ul>
